...
{
	const sobel_input *info = (sobel_input *) input;
	unsigned int length = info->length;
	pam *out = info->out;
	pam *padded = info->padded;
	...
	// The outer one ("y") starts at the initial y position and
	//  goes until it runs out of pixels in "length" or
	//  there are no more pixels to use.
	// The inner one ("x") starts at the given x at the start and
	//  0 afterwards.
	// It also tries to read while there is length or until
	//  it runs out of pixels
	//
	// The tricky thing - initial_x. It is the given one at the start,
	//  but set to 0 after the first loop
	for (unsigned int y = info->start_y; length > 0 && y < out->height; ++y, start_x = 0) {
		// Length is reduced when x is increased
		for (unsigned int x = start_x; length > 0 && x < out->width; ++x, --length) {
			value_x = 0.0;
			value_y = 0.0;
			// Loops might be slow, but didn't slow down terribly
			//  at 15360 x 8640 pixels, so I guess that's fine?
			for (int i = -1; i <= 1; ++i) {
				for (int j = -1; j <= 1; ++j) {
					// +1 for padding
					value_x += kernel_x[i + 1][j + 1] *
							   padded->image[y + i + 1][x + j + 1];
					value_y += kernel_y[i + 1][j + 1] *
							   padded->image[y + i + 1][x + j + 1];
				}
			}

			magnitude = sqrt(pow(value_x, 2) + pow(value_y, 2));
			// I am a bit paranoid about these things.
			// sqrt SHOULDN'T produce values less than 0, but still.
			if (magnitude < 0)
				magnitude = 0;
...
void sobel(pam *in, pam *out, unsigned short n_threads)
{
	...
	// The algorithm breaks the matrix into essentially strips of pixels,
	//  starting at (start_x, start_y)
	// spanning "length" pixels.
	// Every strip is handled by a thread
	// The last thread can have more pixels than the others
	//  as it includes the leftover pixels
	...
	// Fill the data on the last thread, since
	//  it can have more pixels than the others
