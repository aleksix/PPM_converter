...
int ppm_2_pgm(pam *in, pam *out)
{
	...
	for (unsigned int y = 0; y < in->height; ++y) {
		...
		for (unsigned int x_in = 0; x_in < in->width * in->bpp; x_in += 3, ++x_out) {
			...
			out->image[y][x_out] =
					0.2126 * in->image[y][x_in] +
					0.7152 * in->image[y][x_in + 1] +
					0.0722 * in->image[y][x_in + 2];
		}
	}
	...
	for (unsigned int y = 0; y < in->height; ++y) {
		for (unsigned int x_in = 0; x_in < in->width; ++x_in) {
			for (unsigned int x_out = 0; x_out < 8; ++x_out) {
				...
				// (128 >> x_out) will give the bitmask
				//  for a particular bit (number x_out in this_case)
				// ANDing the value with the mask will tell us
				//  if the bit exists (it will be non-zero)
				// The hack part: logic operations return 1 for true
				//  and 0 for false.
				// So, the bit is checked, and if it is white,
				//  then we multiply it by 255 to get white in PGM
