...
// "max length" is used as a sort of a type descriptor, since
//  it is based on minimums from limits.h
...
int read_pam(const char *filename, pam *out)
{
	...
	if (out->type == PBM_BINARY || out->type == PGM_BINARY ||
		out->type == PPM_BINARY) {
		...
		// PBM and PBM_BINARY are consistent in their representation -
		//  8 pixels in 1 byte
...
{
	int fd = open(filename, O_RDWR | O_CREAT, 0774);
